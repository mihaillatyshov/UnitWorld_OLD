...
{
    for (uint i = 0 ; i < std::min(ARRAY_SIZE_IN_ELEMENTS(IDs), 4ull); i++) {
        if (Weights[i] == 0.0) {
            IDs[i]     = BoneID;
            Weights[i] = Weight;
            return;
    ...
    LOGW("should never get here - more bones than we have space for");
    //CORE_ASSERT();
    //assert(0);
}
...
void Mesh::ReadNodeHeirarchy(float AnimationTime, const aiNode* pNode, const Matrix4f& ParentTransform, size_t AnimID)
{    
    string NodeName(pNode->mName.data);
    
    const aiAnimation* pAnimation = m_pScene->mAnimations[AnimID];
...
void Mesh::BoneTransform(float TimeInSeconds, vector<Matrix4f>& Transforms, size_t AnimID)
{
    Matrix4f Identity;
    Identity.InitIdentity();
    
    float TicksPerSecond = (float)(m_pScene->mAnimations[AnimID]->mTicksPerSecond != 0 ? m_pScene->mAnimations[AnimID]->mTicksPerSecond : 25.0f);
    float TimeInTicks = TimeInSeconds * TicksPerSecond;
    float AnimationTime = fmod(TimeInTicks, (float)m_pScene->mAnimations[AnimID]->mDuration);
